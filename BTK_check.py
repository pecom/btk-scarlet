...
catalog_name = "../relfiles/sample_input_catalog.fits"
...
for ndx in allndx:
    ...
    deblender = btk.deblend.ScarletDeblend(n=1)
    deblend_batch = deblender(blend_batch, njobs=4)
    ...
    scarlet_flux = np.array([np.array(list(scarlet.measure.flux(s[0]).data)) for s in deblend_batch.scarlet_sources])
    ...
    np.save(f'./output/{fname_prefix}truth', full_flux)
    np.save(f'./output/{fname_prefix}realizations', realization_flux)
    np.save(f'./output/{fname_prefix}scarlet', scarlet_flux)
    np.save(f'./output/{fname_prefix}err', boot_err)
